#!/usr/bin/env python3
"""REFONTE RECOMMANDATIONS QCM1 - grandes images, bannières, style jeu"""
import os
...
# Remonter au commentaire /* avant QCM2
idx_end = code.rfind("\n/*", 0, idx_fn_end)

print(f"Remplacement: chars {idx_start} -> {idx_end}")

if idx_start == -1 or idx_end == -1 or idx_fn_start == -1 or idx_fn_end == -1:
    print("ERREUR: fonctions non trouvées")
    exit(1)
...
function RecommandationsQcm1Screen({ nutrition, onBack, onVoirRecettes }) {
  ...
  const RECOS = [
    { key:"legumes",      icon:"/legume2.png",    label:"L\\u00e9gumes frais",      bon:v=>v>=60, badge:"Pas assez",          imgBg:"#EAF3DE", badgeBg:"#EAF3DE", badgeC:"#3B6D11", conseil:"2 portions/jour minimum. Varie les couleurs pour maximiser les vitamines et antioxydants.", detailBg:"#EAF3DE", detailBorder:"#9ACD32", detailC:"#27500A", btnBg:"#639922", profil:"legumes" },
    { key:"fruits",       icon:"/fruit.png",       label:"Fruits",             bon:v=>v>=60, badge:"Pas assez",          imgBg:"#FBEAF0", badgeBg:"#FBEAF0", badgeC:"#72243E", conseil:"2 fruits entiers par jour minimum. Varie les esp\\u00e8ces selon les saisons.", detailBg:"#FBEAF0", detailBorder:"#D4537E", detailC:"#4B1528", btnBg:"#D4537E", profil:"legumes" },
    { key:"legumineuses", icon:"/legumesec.png",   label:"L\\u00e9gumineuses",       bon:v=>v>=50, badge:"Pas assez",          imgBg:"#EFEBE9", badgeBg:"#EFEBE9", badgeC:"#5D4037", conseil:"Lentilles, pois chiches, haricots \\u2014 2 fois/semaine minimum. Riches en fibres et prot\\u00e9ines v\\u00e9g\\u00e9tales.", detailBg:"#EFEBE9", detailBorder:"#795548", detailC:"#3E2723", btnBg:"#795548", profil:"legumineuses" },
    { key:"poisson",      icon:"/poisson.png",     label:"Poisson",            bon:v=>v>=50, badge:"Pas assez",          imgBg:"#E1F5FE", badgeBg:"#E1F5FE", badgeC:"#0C447C", conseil:"2 fois/semaine dont 1 gras (sardine, maquereau). Les conserves comptent aussi !", detailBg:"#E1F5FE", detailBorder:"#378ADD", detailC:"#042C53", btnBg:"#378ADD", profil:"poisson" },
    { key:"feculents",    icon:"/feculent.png",    label:"F\\u00e9culents complets", bon:v=>v>=50, badge:"Pas assez",          imgBg:"#FAEEDA", badgeBg:"#FAEEDA", badgeC:"#633806", conseil:"Pain complet, riz brun, p\\u00e2tes compl\\u00e8tes \\u2014 plus riches en fibres.", detailBg:"#FAEEDA", detailBorder:"#BA7517", detailC:"#412402", btnBg:"#BA7517", profil:"rapide" },
    { key:"charcuterie",  icon:"/charcuterie.png", label:"Charcuterie",        bon:v=>v<=20, badge:"Tu en manges trop",   imgBg:"#FFF0EB", badgeBg:"#FFF0EB", badgeC:"#993C1D", conseil:"Max 150g/semaine. Class\\u00e9e canc\\u00e9rig\\u00e8ne groupe 1 par l'OMS. Remplace par des oeufs ou de la volaille.", detailBg:"#FFF0EB", detailBorder:"#FA8072", detailC:"#4A1B0C", btnBg:"#c4622d", profil:"charcuterie", inverse:true },
    { key:"fastFood",     icon:"/fast_food.png",   label:"Fast food",          bon:v=>v<=20, badge:"Trop fr\\u00e9quent",     imgBg:"#FCEBEB", badgeBg:"#FCEBEB", badgeC:"#791F1F", conseil:"Li\\u00e9 \\u00e0 l'ob\\u00e9sit\\u00e9 et au diab\\u00e8te. M\\u00eame un repas simple maison vaut bien mieux.", detailBg:"#FCEBEB", detailBorder:"#E24B4A", detailC:"#501313", btnBg:"#E24B4A", profil:"fastFood", inverse:true },
    { key:"sucres",       icon:"/sucrerie.png",    label:"Sucreries",          bon:v=>v<=20, badge:"Trop fr\\u00e9quent",     imgBg:"#FBEAF0", badgeBg:"#FBEAF0", badgeC:"#72243E", conseil:"Les sucres ajout\\u00e9s favorisent les caries et le diab\\u00e8te. Remplace par des fruits frais.", detailBg:"#FBEAF0", detailBorder:"#D4537E", detailC:"#4B1528", btnBg:"#D4537E", profil:"sucres", inverse:true },
  ];
    ...
    <div style={{ position:"fixed", inset:0, background:"#FFF8F0", fontFamily:"Arial, sans-serif", overflowY:"auto" }}>
      ...
      <div style={{ position:"relative", height:200, overflow:"hidden", borderBottom:"3px solid #222" }}>
        ...
        <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", justifyContent:"flex-end", padding:"20px 24px" }}>
          ...
          <div style={{ fontSize:13, color:"#FFF5EE", opacity:0.85 }}>
            Bas\\u00e9es sur tes habitudes alimentaires d\\u00e9clar\\u00e9es
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:12, padding:"14px 18px", background:"#FFF8F0", borderBottom:"1.5px solid #f0e8de" }}>
        {[
          ...
          [bienFait.length,    "D\\u00e9j\\u00e0 bien",  "#9ACD32"],
      ...
      <div style={{ padding:"16px 18px" }}>
        ...
        {aAmeliorer.length === 0 && (
          <div style={{ background:"white", border:"1.5px solid #C0DD97", borderRadius:12, padding:"24px 20px", textAlign:"center", marginBottom:20 }}>
            ...
            <p style={{ fontSize:12, color:"#777", lineHeight:1.7, margin:0 }}>Tes r\\u00e9sultats sont conformes aux recommandations du PNNS !</p>
          </div>
        )}

        {/* Banni\\u00e8re basse PNNS avec fond_vert3.png */}
        <div style={{ position:"relative", height:100, overflow:"hidden", borderRadius:12, border:"1.5px solid #ede8e0" }}>
          ...
          <div style={{ position:"absolute", inset:0, padding:"14px 18px", display:"flex", alignItems:"center", gap:20 }}>
            <div style={{ fontFamily:"Arial Black, Arial", fontSize:13, color:"white", textShadow:"1px 1px 0 rgba(0,0,0,0.2)", flexShrink:0, lineHeight:1.3 }}>Rep\\u00e8res<br/>PNNS</div>
            <div style={{ display:"flex", flexWrap:"wrap", flex:1, gap:"5px 16px" }}>
              {[["Fruits & l\\u00e9gumes","5 portions/jour"],["Poisson","2x/sem dont 1 gras"],["L\\u00e9gumineuses","2x/semaine"],["Charcuterie","Max 150g/sem"]].map(([k,v]) => (
                <div key={k}>
